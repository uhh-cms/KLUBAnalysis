...
def write_condor_file(d, condor_name, shell_exec, c_exec, py_exec,
    ...
    with open(condor_name, 'w') as s:
        s.write('\n'.join(('Universe = vanilla',
                           ...
                           'input = {}'.format(c_exec),
                           'output = ' +
                           paths['out'].format(qvars[0], qvars[1]),
                           'error = ' +
                           paths['err'].format(qvars[0], qvars[1]),
...
def skim_ntuple(FLAGS, curr_folder):
    ...
    py_exec = os.path.join(os.path.dirname(os.path.dirname(
        FLAGS.config)), 'scripts', 'check_outputs_uhh.py')
    ...
    if not os.path.exists(FLAGS.input_folder):
        print('The input folder {} does not exists. Exiting.'.format(
            FLAGS.input_folder))
        sys.exit(1)
    ...
    def accumulate(l): return [sum(l[:y]) for y in range(1, len(l)+1)]
    inputlists = [inputfiles[x-y:x]
                  for x, y in zip(accumulate(nfiles_per_job), nfiles_per_job)]
    ...
    with open(job_name_shell, 'w') as s:
        ...
        def yes_or_no(s): return '1' if bool(s) else '0'
        ...
        if FLAGS.doSyst:
            sys_command, sys_comment = double_join('skimOutputter.exe',
                                                   os.path.join(
                                                       jobs_dir, io_names[1]),
                                                   os.path.join(
                                                       jobs_dir, 'syst_'+io_names[1]),
                                                   FLAGS.config,
                                                   yes_or_no(FLAGS.isdata))

            sys_command += (' ' + '>& ' +
                            os.path.join(jobs_dir, 'syst_' + io_names[2]))
            sys_comment += ('# extra:\t' + '>& ' +
                            os.path.join(jobs_dir, 'syst_' + io_names[2]))
            s.write(sys_command + '\n')
            s.write(sys_comment + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input_folder',
                        dest='input_folder', default='none', help='input folder')
    parser.add_argument('--exec_file', required=True,
                        help='folder where the C++ skimmer executable is stored')
    ...
    parser.add_argument('-q', '--queue_mode', default='long',
                        choices=('short', 'long'), help='batch queue mode')
    parser.add_argument('--rt', '--runtime',  dest='runtime',
                        help='max runtime in hours',                   default=3, type=float)
    parser.add_argument('-r', '--resub', action='store_true',
                        help='resubmit failed jobs')
    parser.add_argument('-v', '--verb', default=0, type=int, help='verbose')
    parser.add_argument('-d', '--isdata', default=0,
                        type=int, help='data flag')
    ...
    parser.add_argument('-H', '--hadd', default='none',
                        help='hadd the resulting ntuples')
    parser.add_argument('-c', '--config', default='none',
                        help='skim config file')
    parser.add_argument('-n', '--njobs', default=100,
                        type=int, help='number of skim jobs')
    parser.add_argument('-k', '--kinfit', dest='dokinfit',
                        default=1, type=int, help='run HH kin fitter')
    parser.add_argument('-m', '--mt2', dest='domt2', default=1,
                        type=int, help='run stransverse mass calculation')
    parser.add_argument('-y', '--xsscale', default='1.0',
                        help='scale to apply on XS for stitching')
    parser.add_argument('-Z', '--htcutlow', default='-999.0',
                        help='HT low cut for stitching on inclusive')
    parser.add_argument('-z', '--htcut', default='-999.0',
                        help='HT cut for stitching on inclusive')
    parser.add_argument('-e', '--njets', default='-999',
                        help='njets required for stitching on inclusive')
    parser.add_argument('-t', '--toprew', default=0, type=int,
                        help='is TT bar sample to compute reweight?')
    parser.add_argument('-b', '--topstitch', default='0',
                        help='type of TT gen level decay pruning for stitch')
    parser.add_argument('-g', '--dystitching', default=0,
                        type=int, help='whether to use DY stitching or not.')
    parser.add_argument('-a', '--ishhsignal', default=0,
                        type=int, help='isHHsignal')
    parser.add_argument('--BSMname', default='none',
                        help='additional name for EFT benchmarks')
    ...
    parser.add_argument('--order', default='nlo',
                        help='order of reweight: lo/nlo')
    parser.add_argument('--uncert', default='0',
                        help='uncertainty on the reweight coeffs')
    parser.add_argument('--cms_fake', default='0',
                        help='invert some couplings for 2017/2018')
    parser.add_argument('--kl', dest='klreweight', default='-999.0',
                        help='invert some couplings for 2017/2018')
    parser.add_argument('--kt', dest='ktreweight',
                        default='-999.0', help='kt for dynamic reweight')
    parser.add_argument('--c2', dest='c2reweight',
                        default='-999.0', help='c2 for dynamic reweight')
    parser.add_argument('--cg', dest='cgreweight',
                        default='-999.0', help='cg for dynamic reweight')
    parser.add_argument('--c2g', dest='c2greweight',
                        default='-999.0', help='c2g for dynamic reweight')
    parser.add_argument('--susy', dest='susyModel',
                        default='NOTSUSY', help='name of susy model to select')
    parser.add_argument('--pu', dest='PUweights',
                        default='none', help='name of susy model to select')
    parser.add_argument('--nj', dest='DY_nJets', default='-1',
                        help='number of gen Jets for DY bins')
    parser.add_argument('--nb', dest='DY_nBJets', default='-1',
                        help='number of gen BJets for DY bins')
    parser.add_argument('--DY', default=0, type=int,
                        help='if it is a DY sample')
    parser.add_argument('--ttHToNonBB', default=0, type=int,
                        help='if it is a ttHToNonBB sample')
    parser.add_argument('--hhNLO', default=False,
                        action='store_true', help='if it is an HH NLO sample')
