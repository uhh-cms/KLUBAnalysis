...
#            print os.path.exists (filename)
            ...
            if "R__unzip: error" in log_lines or "... SKIM finished, exiting." not in log_lines:
                if opt.verb:
                    print num, 'log file contains errors'
                missing.append(num)
        ...
        with open('%s/missing.txt' % jobsDir, 'w') as missing_file:
            missing_file.write('\n'.join(missing))

        condorResubmitFile = open('%s/condorLauncher_resubmit.sh' % jobsDir, 'w')
        condorResubmitFile.write('Universe = vanilla\n')
        ...
        command = 'condor_submit ' + jobsDir + '/condorLauncher_resubmit.sh -batch-name ' + opt.output.split('/')[-1] + '_resubmit'
    ...
    if opt.njobs > len(inputfiles) or opt.njobs < 1:
        opt.njobs = len(inputfiles)
    ...
    for listname in inputlists:
        ...
        if opt.ishhsignal:
            ...
            command += " 0 "
        command += (" " + opt.njets)
        command += (" " + opt.EFTrew + " " + opt.order + " " + opt.uncert + " " + opt.cms_fake + " " + opt.klreweight +
                    " " + opt.ktreweight + " " + opt.c2reweight + " " + opt.cgreweight + " " + opt.c2greweight)
    ...
    condorFile = open('%s/condorLauncher.sh' % jobsDir, 'w')
    condorFile.write('Universe = vanilla\n')
    condorFile.write('Requirements = ( OpSysAndVer == "CentOS7" )\n')
    condorFile.write('+RequestRuntime = {}\n'.format(int(opt.runtime * 3600) - 1))
    condorFile.write('Executable = '+jobsDir + '/skimJob_$(ProcId).sh\n')
    condorFile.write('Log = ' + jobsDir + '/condor_$(ClusterId).$(ProcId)_job_$(ProcId).log\n')
    condorFile.write('Output = ' + jobsDir + '/condor_$(ClusterId).$(ProcId)_job_$(ProcId).out\n')
    condorFile.write('Error = ' + jobsDir + '/condor_$(ClusterId).$(ProcId)_job_$(ProcId).error\n')
    condorFile.write('queue %d\n' % len(inputlists))
    condorFile.close()

    command = 'condor_submit ' + jobsDir + '/condorLauncher.sh -batch-name ' + opt.output.split('/')[-1]
    if opt.sleep:
        time.sleep(0.1)
    if opt.dry:
        print(command)
